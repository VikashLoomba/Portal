...
2. **Transparent sudo** — a `sudo` wrapper shim + `SUDO_ASKPASS` helper: when an agent (no controlling terminal) runs
   `sudo <cmd>`, the Mac dialog pops, the password flows down the existing pipe directly into sudo's
   askpass pipe, and the command proceeds. Any session with a controlling terminal is untouched.
...
| C4 | **`portald keychain` subcommand family (box side)** | New `keychain` case in portald's dispatch. `portald keychain run --label <L> (--env NAME \| --stdin) -- <argv...>`: dials the cmd socket (same fanout/single-agent refusal semantics as `runClip`), sends the C3 line, and on `ok` **delivers without the calling shell ever seeing the secret**: `--env` validates `NAME` against `[A-Za-z_][A-Za-z0-9_]*` then `syscall.Exec`s the child with `NAME=<secret>` appended to the environment (no lingering parent); `--stdin` runs the child via os/exec with the secret + `\n` as its entire stdin, propagating the child's exit code. Exit codes: child's on success; **111** = denied, **112** = timeout, **2** = usage; stderr messages are agent-legible ("portal keychain: denied by user on the Mac"). Portald emits box-local `multiple-clients` when fanout finds more than one distinct agent and `invalid-response` when the selected agent's reply is malformed; neither reason is sent by the Mac. `portald keychain askpass [prompt...]`: sudo/ssh invoke it with the prompt as argv, so arguments are opaque prompt text — **except a LONE `help`/`-h`/`--help`, which prints help and never requests** (amended after a coding agent ran `portal keychain askpass --help` expecting usage, was served a real approved secret on stdout, and the password entered its context — the exact outcome §1 promises to prevent). It additionally **refuses to request at all unless the invoking parent is an askpass consumer** (`sudo`/`sudoedit`/`sudo-rs`/`ssh`; override `PORTAL_ASKPASS_ALLOW_ANY=1`), failing closed on an undeterminable parent. That gate stops ACCIDENTS, not attackers — a hostile same-uid process can present any parent, which §1's threat model already excludes. It sends `Mode="askpass"`, `Target=<prompt>`, and on `ok` prints the secret + `\n` to stdout (consumed by sudo's pipe — never by the agent) or exits 1 on deny. Requester context: read `/proc/<ppid>/cmdline` (self's parent), format `pid <ppid>: <cmdline>`, truncate to 300 bytes. Top-level `keychain` and `keychain run --help` are **agent-first**, with copy-pasteable examples INCLUDING the quoting subtlety (`portal keychain run --label "staging admin" --env PW -- sh -c 'curl -d "pass=$PW" …'` — single quotes so the *child* shell expands `$PW`). |
| C5 | **Box shims + shell startup (clipshim `Version` 3→7)** | Three shims live in `internal/clipshim` (`credshim.go`, same package — the package is portal's box-shim deployer, name kept to avoid churn): (i) **`~/.local/bin/portal`** — passthrough `exec "$_portald" "$@"` with a clear error if portald is missing, so the agent-facing name matches the Mac (`portal keychain …`); (ii) **`~/.local/bin/portal-askpass`** — `exec "$_portald" keychain askpass "$@"`; (iii) **`~/.local/bin/sudo`** — the seamless wrapper. All real-binary fallbacks resolve through a quoted `IFS=:` loop that treats PATH as data, skips empty/self entries, and rejects same-file aliases; PATH never enters `sh -c`. The sudo shim first passes through verbatim whenever any standard stream is a TTY or `/dev/tty` is reachable. Only after that safety branch, an unset/empty `SUDO_ASKPASS` self-defaults to executable `~/.local/bin/portal-askpass` and is exported; an existing user value is never overridden. It passes through when the selected helper is unavailable or when sudo's leading options contain a short conflict flag (`A/S/n/e/h/V/K/k/v`, including bundles) or exact long conflict (`--askpass/--stdin/--non-interactive/--edit`). The scan stops at `--` or the first command token; only a fully detached invocation otherwise receives `-A`. All three carry the versioned `Marker`, while backup ownership uses the stable unversioned `Installed by portal` prefix: upgrades never copy an older portal shim into an empty backup slot, and `Remove()` restores only genuine user backups while deleting already-polluted portal-owned backups. Shell startup has three independently convergent ranges: the bottom PATH block and the SUDO_ASKPASS block append to `~/.bashrc`, `~/.zshrc`, `~/.zshenv`, and `~/.profile` with create-if-missing behavior, and append to `~/.bash_profile` / `~/.bash_login` **only when those files already exist** so portal never creates a login file that shadows `~/.profile`; a distinct early PATH block is truncate-prepended to `~/.bashrc` above the distro interactive guard for sshd-sourced non-interactive bash, preserving inode and permissions, while the bottom block remains to re-win interactively. `Remove()` strips all three ranges from all six files. |
...
| C10 | **Timeout budget (human-scale, unlike clip)** | One Mac request has a total dialog budget of **115s**; the first prompt receives the remaining seconds and a Forget re-prompt receives only what remains. This is < agent `credTimeout` **130s** (host.Call wait) < cred `Verb.Deadline` **135s** (cmd-socket) < portald keychain socket-read deadline **140s** (outer bound). A pending cred waiter blocks nothing: heartbeats and other services interleave via the Serve loop's merged outbox, so a human pause cannot trip the 12s heartbeat reconnect (unlike clip, whose 9s ceiling exists for paste UX, not protocol safety). |
...
| `internal/clipshim/clipshim.go` (+tests) | C5: `Version = "7"`, conditional bash login files, early non-interactive bash PATH block, and three-range removal. |
...
| u8 | **Hardening**: refine sudo's leading-option scan and bump shim `Version` to 5; add direct `internal/osa` escaping tests; run `GOFLAGS=-trimpath make test` (CI parity), `go test -race ./...`, gofmt, and greps — secret never in argv (`security -i` only; no `add-generic-password …-w <arg>` form), secret never logged/formatted, no new deps (`go.mod` unchanged), exit-criteria sweep. |
| u9 | **Adversarial hardening**: replace shell-reparsed PATH resolvers; fail safe for every controlling-terminal sudo session; recognize bundled sudo conflicts; keep askpass argv opaque; preserve user SUDO_ASKPASS; parse real osascript timeout bytes; cap total Mac dialog time at 115s; sanitize disabled audits; bump shim `Version` to 6. |
| u10 | **Linux shell-startup coverage**: conditionally manage existing `.bash_profile` / `.bash_login`; prepend an early PATH range to `.bashrc` for sshd-sourced non-interactive bash; let detached sudo self-default an empty SUDO_ASKPASS; key backup/restore ownership on the version-independent marker prefix so upgrades never pollute backup slots and uninstall deletes (not restores) portal-owned backups; bump shim `Version` to 7. |
...
5. **C5:** all three shims deploy with marker + backup; the four standard rc files keep create-if-missing behavior while `.bash_profile` / `.bash_login` are never created; the early `.bashrc` range is top-placed, idempotent, and inode/mode preserving; `Remove()` restores a pre-existing sudo/portal binary, deletes a portal-owned backup instead of restoring it, and strips all three ranges from all six rc files; backup ownership greps the version-independent prefix so an upgrade over an older shim creates no backup; PATH text is never shell-evaluated; the sudo wrapper's passthrough matrix (controlling terminal / leading or bundled conflicts / command flags / self-defaulted, user-selected, or unavailable SUDO_ASKPASS / no-real-sudo) is asserted against the script text and behavior; `Version = "7"` re-converges existing v6 boxes.
...
| `sudo` wrapper breaks or hijacks real sudo usage | Any reachable controlling terminal forces byte-for-byte passthrough, including redirected stdin; leading and bundled sudo conflicts pass through while command flags do not suppress askpass. Tests pin the matrix; backup/restore via deployShim and the feature gate remain escape hatches. |
...
| rc-block migration on existing installs | The askpass export and early bash PATH logic have their OWN stable marker ranges (C5); existing login files are updated but never created, a pre-existing askpass value is preserved, and `Version=7` re-deploys the self-defaulting sudo shim via the existing marker grep. |
...
3. **Scenario 2 sudo:** `ssh vikash-system sudo whoami` (no controlling terminal) → Mac dialog ("piped to sudo") → `root`; interactive `portal exec` then `sudo whoami`, including redirected stdin, → in-terminal password prompt as before (wrapper passthrough).
...
6. **Timeout:** let a dialog sit for its 115s request budget → auto-dismiss, box exits 112.
7. **Uninstall:** `portal uninstall` → box `sudo`/`portal` shims removed/restored, all three rc marker blocks stripped from all managed startup files, `SUDO_ASKPASS` gone from fresh shells.
