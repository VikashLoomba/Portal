...
func TestHandleFeatures(t *testing.T) {
	t.Run("get defaults on", func(t *testing.T) {
		...
		for _, f := range []string{config.FeatureClipImage, config.FeatureClipText, config.FeatureNotify, config.FeatureExec, config.FeatureCred, config.FeatureCredTouchID} {
			if v, ok := got[f]; !ok || !v {
