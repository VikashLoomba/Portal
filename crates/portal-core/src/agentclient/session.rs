//! One protocol session + the reconnect supervisor loop.
//!
//! Two structural rules:
//! - the write side is single-owner: handlers enqueue [`Outbound`] frames on
//!   an mpsc handle rather than sharing a mutex-latched encoder, so a send on
//!   a closed channel is unrepresentable instead of caught;
//! - frames are read by a dedicated task feeding a channel, because
//!   `read_exact` is not cancel-safe inside `select!`.
...
impl Client {
    ...
    async fn drive_session<W: AsyncWrite + Send + Unpin>(
        ...
        // Hello → HelloAck. The handshake read is bounded by the heartbeat
        // timeout, so a wedged remote shell cannot block us indefinitely.
        write_frame(
            stdin,
