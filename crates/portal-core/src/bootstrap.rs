//! Embedded-agent → remote-cache lifecycle.
//!
//! 1. Probe `~/.cache/portal/agent-<git-sha>` by BOTH byte-count and sha256.
//!    Length alone is not enough, and this has bitten before: a same-size
...
/// portal-cli; `make agents` cross-builds the musl targets and the Makefile
/// verifies they actually embedded).
#[derive(Clone)]
...
fn path_prepend_snippet() -> String {
    format!(
        "{PATH_MARKER_START}\n\
         # Ensures portal's shims (~/.local/bin/portal, xclip, wl-paste, pbpaste,\n\
         # wl-copy, pbcopy, sudo, portal-askpass, xdg-open) win on PATH.\n\
         {DEDUP_PREPEND}\n\
         {PATH_MARKER_END}"
    )
}
