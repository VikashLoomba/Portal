//! Remembered-secret storage seam.
//!
//! Items are portal-namespaced ordinary generic passwords, reachable either via
//! the `security-framework` crate or `security add-generic-password -U` /
//! `find-generic-password -w` / `delete-generic-password`. They must STAY
//! ordinary generic passwords: Touch ID gates portal's decision to RELEASE a
//! secret, it does not re-bind the Keychain item to biometrics.
