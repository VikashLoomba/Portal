...
//! feature.
//!
//! The guarantee: the secret never enters the agent's
...
//! - this module — wire limits, modes, and deny reasons (these cross the v4
//!   protocol, so both peers must agree byte-for-byte).
...
/// `CredResponse.err` — see [`portal_proto::messages`]. The box-side runner
