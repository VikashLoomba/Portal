//! Mac clipboard access.
//!
//! Do NOT reintroduce `osascript` here. Shelling out to AppleScript for every
//! probe/read means coercing the pasteboard through a temp file, and the
//! coercion cost on a large image blows any sane per-read deadline — that is
//! the failure mode this module exists to avoid, not a style preference.
//!
//! Reads go through NSPasteboard IN-PROCESS (objc2-app-kit):
