...
var featureNames = []string{config.FeatureClipImage, config.FeatureClipText, config.FeatureNotify}

func newFeaturesCmd(a *app.App) *cobra.Command {
	return &cobra.Command{
		Use:   "features [name on|off]",
		Short: "Show or toggle the clip-image/clip-text/notify capability gates",
...
func runFeatures(ctx context.Context, w, errw io.Writer, a *app.App, args []string) error {
	...
	case 2: // SET
		...
		if !knownFeature(name) {
			fmt.Fprintf(errw, "unknown feature: %s (known: clip-image, clip-text, notify)\n", name)
			return usageErr{}
		}
