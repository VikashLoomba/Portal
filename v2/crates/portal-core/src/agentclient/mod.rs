...
//! transport's stream (port of pkg/agentclient).
//!
//! Semantics carried over from v1 (each pinned by a test):
...
/// Mirrors the v1 auto-registered handler set.
/// The services this client advertises in Hello (v4 symmetric negotiation).
/// HONEST advertisement (S4): only what the daemon actually serves today.
/// A v1 Go agent checks `ClientHas(svc)` and answers "none"/"no-client"
/// IMMEDIATELY for anything absent — so not advertising is the graceful
/// degrade, advertising-without-a-handler is a 9s timeout. `clipwrite` and
/// `cred` join this map when their Mac-side handlers land.
