//! Derived file/path locations (port of `internal/app/paths.go`).
//!
//! The v1 env-var test seams are kept with the SAME names so the existing
//! staging workflows carry over: `PORTAL_CONFIG_DIR`, `PORTAL_API_SOCK`.
//! v2 drops `PORTAL_SOCK` entirely — there is no ControlMaster socket; the
