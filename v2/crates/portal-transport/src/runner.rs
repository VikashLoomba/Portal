//! Subprocess runner seam (port of `pkg/run`). Every ssh/lsof invocation goes
//! through a `Runner` so transports and port-listers are unit-testable with a
//! scripted fake — the same discipline that let the Go sshctl encode its
//! empirically-derived ssh gotchas in tests.
