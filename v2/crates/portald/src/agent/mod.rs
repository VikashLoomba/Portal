//! The agent serve loop (port of pkg/agent/server.go, narrowed to v2's
//! service set): speaks v4 frames on stdin/stdout, watches loopback
//! listeners, applies clipsync updates to the store, and relays cmd-socket
//! events (notify/open) up the pipe.
//!
//! Contracts carried over from v1:
