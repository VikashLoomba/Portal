...
/// ships in the repo (read from the checkout if present) — a release without
/// a verifiable signature FAILS the upgrade when a signature is present but
/// doesn't verify (never silently skip).
async fn verify_signature(
    ...
    runner
        ...
        .await
        .map_err(|e| e.to_string())?;
    // Public key: from the repo checkout (v2/minisign.pub). If unavailable,
    // we CANNOT verify — fail loudly rather than pretending.
    let pubkey = std::env::var("PORTAL_MINISIGN_PUB")
        .or_else(|_| {
            std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/../../minisign.pub"))
        })
        .map_err(|_| {
            "release is signed but no minisign public key is available (PORTAL_MINISIGN_PUB)"
                .to_string()
        })?;
    let out = runner
        .run(
