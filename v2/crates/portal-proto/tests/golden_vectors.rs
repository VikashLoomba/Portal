//! Cross-language golden vectors: every `docs/vectors/protocol_*.hex` frame
//! (generated by the Go implementation, pkg/protocol) must decode AND
//! re-encode byte-exactly. This is the wire-compatibility contract that lets
//! a Rust `portal` drive a deployed Go `portald` during rollout.
//!
//! (The `exec_*.hex` vectors cover the localapi exec WebSocket frames — a
//! different schema, ported with portal-core's localapi later.)
