//! Wire message types. Field ORDER and serde renames must match the Go struct
//! declarations in `pkg/protocol/messages.go` exactly — ciborium emits map keys
//! in declaration order and the golden-vector tests assert byte-exact
//! re-encoding. Do not reorder fields.
...
/// and spliced inline (Go `cbor.RawMessage` ⇔ `ciborium::Value`).
