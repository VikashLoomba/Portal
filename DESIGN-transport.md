...
## 6.1 Amendment fast-follows (principal-recorded, non-blocking — address in Stage 5 preamble)

1. **Proxy-path dial/handshake deadlines:** `dialViaProxyJump`'s per-hop `prev.Dial` channel opens
   and every `ssh.NewClientConn` handshake are unbounded (x/crypto applies `ClientConfig.Timeout`
   only inside `ssh.Dial`); a black-holed bastion hangs `Ensure` while holding `mu`, wedging
   Health/status. Fix: `prev.DialContext(ctx, ...)` + a watchdog closing the raw conn on
   ctx.Done()/timer around each handshake.
2. **Hop-level ProxyCommand:** a hop whose resolved config carries `ProxyCommand` is silently
   direct-dialed (OpenSSH would honor it; common cloudflared/teleport bastion pattern). Minimum
   fix: a clear error (`proxyjump hop %q uses ProxyCommand, which native does not support for
   hops`); full support is a candidate for a later stage.
3. **ProxyCommand stderr:** discarded today (`cmd.Stderr` nil), so helper failures surface as bare
   `handshake ...: EOF`. Fix: capped buffer on the process closer, included in the
   handshake-failure error.

Watch items for Stage 5/6 (from the principal's assessment): the target is resolved ONCE at `New`
while hops re-resolve at every `Ensure`/redial (asymmetry to keep or document deliberately); with
native selected, every CLI invocation execs `ssh -G` at App construction (5s worst case, degraded
via errTransport); `WithProxyCommandDialer` exports an Option over an unexported func type —
Stage 6 extraction must choose that type's public shape deliberately.
